...
Use this skill after a design direction exists. It is the independent taste, polish, and verification pass: inspect the implemented UI, name what feels wrong, apply scoped fixes when asked, and verify the result in a browser when practical. The canonical first-draft and premium-audit workflow lives in `design`.
...
- `review-plan`: Critique an existing UI/UX plan, wireframe, or design spec before implementation.
- `implement-polish`: Apply scoped polish fixes to an approved or already implemented direction.

If the request wants fresh directions, concepts, a design system, or a premium-experience direction before implementation, route to `design`. If a premium audit is requested for an implemented surface, `design` owns the five-layer premium audit; use this skill afterward for independent pixel-level verification.
...
## Anti-slop and production-defect checklist
...
Check the following during `audit`, `review-plan`, and `implement-polish` when relevant:
