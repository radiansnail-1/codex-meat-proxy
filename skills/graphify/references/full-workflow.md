...
- If `skipped_sensitive` is non-empty: report only the count. Do not print sensitive-looking file names; inspect or disclose them only if the user explicitly requests that troubleshooting step.
...
Load files from `graphify-out/.graphify_uncached.txt`. Build chunks using both file count and input size:

- A text chunk may contain at most 8 files and approximately 10,000 words. Start a new chunk as soon as adding the next file would exceed either limit. A single text file over 10,000 words gets its own chunk.
- Estimate words from the readable text content, not the corpus-wide average. A paper or other content type whose text size cannot be estimated cheaply gets its own chunk.
- Each image gets its own chunk because vision needs separate context.
- Group related files from the same directory when they fit inside both limits, so useful cross-file relationships remain discoverable.
...
- Warn and exclude failed, timed-out, or invalid chunks.
- If more than half of the dispatched chunks fail, stop without replacing the existing graph.
- Merge successful arrays in memory and write `graphify-out/.graphify_semantic_new.json` atomically.
- Write the exact source paths covered by successful chunks to `graphify-out/.graphify_successful.txt`; failed or omitted files must not appear there.
- Use actual tool-reported token usage when available; otherwise keep the schema's zero placeholders and label token totals unavailable rather than inventing them.
...
successful_files = [line for line in Path('graphify-out/.graphify_successful.txt').read_text(encoding=\"utf-8\").splitlines() if line]
saved = save_semantic_cache(new.get('nodes', []), new.get('edges', []), new.get('hyperedges', []), root='INPUT_PATH', allowed_source_files=successful_files, prompt_file='SPEC_PATH')
...
"
```
Clean up temp files: `rm -f graphify-out/.graphify_cached.json graphify-out/.graphify_uncached.txt graphify-out/.graphify_successful.txt graphify-out/.graphify_semantic_new.json`
