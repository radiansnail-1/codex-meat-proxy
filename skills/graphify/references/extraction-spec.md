...
- Keep extraction selective and bounded. Prefer durable, high-signal entities and relationships over exhaustive coverage. Emit at most 8 nodes per source file, 64 nodes total per chunk, and 96 edges total per chunk. Stop adding lower-value concepts or edges when a limit is reached; never exceed a limit to be "complete".
